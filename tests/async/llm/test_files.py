...
async def test_generate_with_file(model: LLMModel, example_assets: pathlib.Path) -> None:
...
async def test_generate_with_multiple_files(model: LLMModel, example_assets: pathlib.Path) -> None:
...
async def test_generate_with_unexisting_file(model: LLMModel, tmp_path: pathlib.Path) -> None:
    my_file = tmp_path / "foo"
    with pytest.raises(FileNotFoundError):
        await model.generate("What is written in the file?", files=[my_file])


async def test_generate_with_not_a_file(model: LLMModel, tmp_path: pathlib.Path) -> None:
