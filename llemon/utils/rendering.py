...
class Rendering:
    ...
    def __init__(self, bracket: str) -> None:
        ...
        self._async_regex = re.compile(rf"{re.escape(open)}\s*!\s*(.*?){re.escape(close)}")
    ...
    def predicate(cls, function: Callable[..., Any]) -> Callable[..., Any]:
        function = to_async(function)

        @pass_context
        async def test(ctx: Context, *args: Any, **kwargs: Any) -> Any:
            return await function(dict(ctx), *args, **kwargs)
    ...
    async def render(self, text: str, context_dict: dict[str, Any] | None = None, /, **context_kwargs: Any) -> str:
    ...
    async def _evaluate(self, expression: str, context: dict[str, Any]) -> str:
        return await eval(expression, context)