...
from llemon.utils import Cache, Emoji, parallelize, wait_for
...
class Gemini(llemon.LLMProvider):
    ...
    def _config(self, request: GenerateRequest) -> GenerateContentConfig:
        ...
        cache_name = self._cache(request, tools, tool_config)
        if cache_name:
    ...
    def _cache(self, request: GenerateRequest, tools: list[Tool] | None, tool_config: ToolConfig | None) -> str | None:
